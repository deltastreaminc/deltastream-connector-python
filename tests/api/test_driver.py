...
async def test_should_throw_auth_error_if_invalid_token_provided():
    ...
    conn = APIConnection.from_dsn('https://api.deltastream.io/v2?sessionID=123', token_provider)
    mock_api = MagicMock()
...
async def test_should_succeed_if_valid_token_provided_in_url():
    conn = APIConnection.from_dsn('https://_:sometoken@api.deltastream.io/v2?sessionID=123')
    mock_api = MagicMock()
